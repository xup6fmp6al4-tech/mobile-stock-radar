const CACHE='stock-radar-v0.7-numeric';
...
async function injectPatch(request){
  try{
    ...
    if(!html.includes('futures-display-patch.js')){
      html=html.replace('</body>',`<script src="${PATCH}?v=20260906-1"></script></body>`);
    }
...
self.addEventListener('fetch',e=>{
  ...
  if(e.request.mode==='navigate'){
    e.respondWith(injectPatch(e.request));
    return;
  }
  e.respondWith(
